...
def edit_dataframe_numerical(df : pd.DataFrame):
    running = True
    edited_df = df
...
def edit_dataframe_string(df : pd.DataFrame, strings : []):
    columns = df.select_dtypes(include=['object']).columns #this variable is to ensure only numerical columns can be selected for the editing the dataset
    edited_df = df
    ...
    for column in columns:
        for string in strings:
            print(edited_df[column][string])
            edited_df = edited_df[edited_df[column].str.contains(string, case=False, na=False)]
